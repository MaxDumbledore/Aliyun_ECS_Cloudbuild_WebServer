#ifndef WEBSERVER_GENERATOR_H
#define WEBSERVER_GENERATOR_H
...
struct nlohmann_traits {
    ...
    static jwt::json::type get_type(const json& val) {
        ...
        if (val.type() == json::value_t::number_integer)
            return type::integer;
        if (val.type() ==
            json::value_t::number_unsigned)  // nlohmann internally tracks two
                                             // types of integers
            return type::integer;
...
using JwtDecoded=jwt::decoded_jwt<nlohmann_traits>;
...
class TokenGenerator{
    ...
    JwtDecoded decode(const std::string &token);

    bool verify(const JwtDecoded &decodedToken);
};
